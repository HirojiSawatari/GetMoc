program poptrans
  ...
  ! modified by sgy 7/2004 - update heat/salt transport computation
  !		             to use UET,VNT,UES,VNS if these fields
  !			     exist in input netcdf.  (ie, bar<UT> 
  !			     instead of bar<U>bar<T>, etc)
  !
  ! modfied by cas 10/2009 - adjusted for example basins 
  !                          version
  ! fixed a bug by ygliu 04/2013 - at original line 368, changed vnt to vns
  !gx1v3:
  ...
  varstr = (/&
       ...
       'amhtw       1 Atlantic heat transport            F  3  TS Pw                   ',&
...
!                vns(:,:) = vns(:,:)*tarea*dz(k)
        ...
        enddo
!cas for bolus, ues_isop and uet_isop don't exist
uesexist = .false.
uetexist = .false.
...
720  continue
     ...
     do n = 2,nphi
        do k=1,km
           ...
           apsiw(k,n)=apsiw(k,n-1);amhtw(k,n)=amhtw(k,n-1);amhsw(k,n)=amhsw(k,n-1)
           ipsiw(k,n)=ipsiw(k,n-1);imhtw(k,n)=imhtw(k,n-1);imhsw(k,n)=imhsw(k,n-1)
           ppsiw(k,n)=ppsiw(k,n-1);pmhtw(k,n)=pmhtw(k,n-1);pmhsw(k,n)=pmhsw(k,n-1)
        enddo
        !
        do j=1,jmt
           do i=1,imt
